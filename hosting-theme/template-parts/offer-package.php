<?php
/**
 * Template part: Special Offer Package
 *
...
$title = isset( $offer_package['title'] ) ? $offer_package['title'] : '';
$description = isset( $offer_package['description'] ) ? $offer_package['description'] : '';
$price_bdt = isset( $offer_package['price_bdt'] ) ? (float) $offer_package['price_bdt'] : 0;
$price_usd = isset( $offer_package['price_usd'] ) ? $offer_package['price_usd'] : 0;
$features = isset( $offer_package['features'] ) ? $offer_package['features'] : array();
$order_url = isset( $offer_package['order_url'] ) ? $offer_package['order_url'] : '';
$is_popular = isset( $offer_package['is_popular'] ) ? $offer_package['is_popular'] : false;
...
<div class="<?php echo esc_attr( $card_class ); ?> pricing-card" data-price-bdt="<?php echo esc_attr( $price_bdt ); ?>" data-price-usd="<?php echo esc_attr( $usd_amount ); ?>">
    ...
    <?php if ( $is_popular ) : ?>
        <div class="cloud-popular-badge" style="background: linear-gradient(135deg, var(--orange-primary) 0%, var(--orange-light) 100%);">
            <?php echo esc_html( pll__( 'Limited Time' ) ); ?>
        </div>
    <?php endif; ?>

    <div class="cloud-icon">
        <img src="<?php echo esc_url( HOSTING_THEME_URI . '/assets/images/pricing/gift.svg' ); ?>" alt="<?php echo esc_attr( $title ); ?>" style="width: 60px; height: 60px;">
    </div>
